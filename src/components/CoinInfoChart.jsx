...
export default function CoinInfoChart({ width = 720, height = 360 }) {
  ...
  return (
    <div role="img" aria-label="График данных с локального сервера" style={{ width, height }}>
      {loading && <div style={{ padding: 12 }}>Загрузка данных…</div>}
      {error && (
        <div style={{ padding: 12, color: '#b91c1c' }}>Ошибка загрузки: {error}</div>
      )}
      {!loading && !error && (
