...
// Fetch Token Turnover indicators data (ema7, ema21, roc14, zscore14) for a given id
async function fetchIndicatorData(id) {
  const url = `http://localhost:3000/indicators/token-turnover/${id}`;
...
// Token Turnover Indicator Chart
export default function TokenTurnoverIndicatorChart({
  id,
  width = 720,
  height = 800,
  showLabels = true,
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setError(null);
        // запрос с ?count
        const url = `http://localhost:3000/indicators/token-turnover/${id}?count=${encodeURIComponent(count)}`;
        const resp = await fetch(url);
        if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
        const json = await resp.json();
  ...
  return (
    <div
      role="img"
      aria-label="Token Turnover: EMA7/EMA21, ROC14, Z-Score14"
      style={{ width, height }}
    >
