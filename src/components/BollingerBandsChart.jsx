...
export default function BollingerBandsChart({
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setLoading(true);
        setError(null);
        const url = `http://localhost:3000/indicators/${metric}/${id}?count=${encodeURIComponent(count)}`;
        const resp = await fetch(url);
        if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
        const json = await resp.json();
        
        if (!json?.dates || !json?.bb) {
          throw new Error("Некорректный ответ API (ожидалось поле bb)");
        }

        const datesOnly = json.dates.map((d) => String(d).split("T")[0]);
        const combined = datesOnly.map((x, i) => ({
          x,
          upper: Number(json.bb.upper?.[i]),
          middle: Number(json.bb.middle?.[i]),
          lower: Number(json.bb.lower?.[i]),
        }));
  ...
  const [yMin, yMax] = useMemo(
    () => plotDataMulti(displayData, ["upper", "middle", "lower"]),
    [displayData]
  );
  ...
  return (
    <div style={{ width, height, marginTop: 20 }}>
      {loading && <div style={{ padding: 12 }}>Загрузка BB…</div>}
      {error && (
        <div style={{ padding: 12, color: "#b91c1c" }}>
          Ошибка загрузки BB: {error}
        </div>
      )}
      {!loading && !error && (
